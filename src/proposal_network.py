...
class VelocityTrajectoryProposalNetwork(nn.Module):
    ...
    def __init__(
        ...
        head_input_dim = self.encoded_flat_dim + 2
        for dim in hidden_dims:
            mlp_layers.append(nn.Linear(head_input_dim, dim))
            mlp_layers.append(nn.ReLU(inplace=True))
            head_input_dim = dim
        self.mlp = nn.Sequential(*mlp_layers) if len(mlp_layers) > 0 else nn.Identity()

        self.velocity_mean_head = nn.Linear(head_input_dim, self.horizon * 2)
        self.velocity_var_head = nn.Linear(head_input_dim, self.horizon * 2)
    ...
    def forward(
        ...
        if current_velocity is None:
            velocity_vec = torch.zeros((batch_size, 2), dtype=torch.float32, device=dynamic_x.device)
        else:
            velocity_vec = torch.as_tensor(current_velocity, dtype=torch.float32, device=dynamic_x.device)
            if velocity_vec.ndim == 1:
                if velocity_vec.shape[0] != 2:
                    raise ValueError("current_velocity 1D input must have shape (2,)")
                velocity_vec = velocity_vec.unsqueeze(0)
            if velocity_vec.ndim != 2 or velocity_vec.shape[1] != 2:
                raise ValueError("current_velocity must have shape (B, 2)")
            if velocity_vec.shape[0] != batch_size:
                raise ValueError(
                    f"current_velocity batch size must be {batch_size}, got {velocity_vec.shape[0]}"
                )
        ...
        mlp_input = torch.cat([encoded, velocity_vec.to(dtype=encoded.dtype)], dim=1)
